...
static char *BitstreamErrorStr(Bitstream *self) {
  static char err[128];
  snprintf(err, 128, "i: %ld\tsize: %lu", self->i, self->size);
  return err;
}
...
Result BitstreamSlice(Bitstream *self, const Bitstream *src, u64 offset, u64 size) {
  if (offset + size > src->size) { return ERR(BitstreamEoF, BitstreamErrorStr(self)); }
  self->i = 0;
  self->size = size;
...
Result BitstreamReadU8(Bitstream *self, u8 *buf) {
  if (self->i++ == self->size) { return ERR(BitstreamEoF, BitstreamErrorStr(self)); }
  *buf = self->buf[self->i];
  return OK;
}
